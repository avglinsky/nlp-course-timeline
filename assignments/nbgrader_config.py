...
c.ClearSolutions.code_stub = {'python': '# НАЧАЛО ВАШЕГО РЕШЕНИЯ\nraise NotImplementedError() # удалить эту строку в процессе решения\n# КОНЕЦ ВАШЕГО РЕШЕНИЯ', 'matlab': "% ВАШ КОД ЗДЕСЬ\nerror('No Answer Given!')", 'octave': "% ВАШ КОД ЗДЕСЬ\nerror('No Answer Given!')", 'sas': '/* ВАШ КОД ЗДЕСЬ */\n %notImplemented;', 'java': '// ВАШ КОД ЗДЕСЬ'}
c.ClearSolutions.text_stub = 'НАЧАЛО ВАШЕГО РЕШЕНИЯ'
